...
struct SwiftPackage {
    ...
    func performUpdate(_ update: Update, of dependency: Dependency) throws -> Bool {
        ...
        // swiftlint:disable:next line_length
        let regex = try NSRegularExpression(pattern: "\\.package\\s*\\(\\s*url\\s*:\\s*\"\(NSRegularExpression.escapedPattern(for: dependency.url.absoluteString))\"\\s*,\\s*(\(versionRegExString))", options: [.anchorsMatchLines])
